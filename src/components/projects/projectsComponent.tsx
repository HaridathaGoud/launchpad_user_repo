...
const Projectscomponent = (props) => {
    ...
    const getPrjectCardDetails = async (pageNo: number, pageSize: number, type: any, search: any) => {
        setLoader(true);
        ...
        let response = await get(`User/Projects/${props.pjctType}/${props.pageSize}/${skip}/${search}`);
        if (response) {
    ...
    return (
        <div className='container mx-auto max-sm:px-3 mt-6'> 
            ...
            <div className="">
                ...
                <div className={`flex justify-between mb-4 md:items-center ${props.pageSize >= 9 ? 'max-sm:flex-col gap-3' :''}`}>

                    <h2 className="font-semibold text-xl lg:text-2xl text-secondary  ">{pjctTypes[props.pjctType]} <span className='text-primary'> {process.env.REACT_APP_OFFERING_TITLE}s</span> Projects</h2>
                    {props.pageSize < 9 && <Link to={`/allprojects/${props.pjctType}`} className='text-primary text-base font-medium'>View All</Link>}
                    {props.pageSize >= 9 && <div className="mb-3 d-flex">
                        <div className="relative">
                            <input
                                ...
                                ref={endedIgosRef}
                            />
                            <span className="icon md search absolute right-4 bottom-4 cursor-pointer md:w-72" onClick={() => handleSearchIcon(search)} />
                        </div>
                    </div>}
                </div>
                <div className="text-center">
                    {(cardDetails?.length == 0 && loader) && (
                        ...
                        {cardDetails?.length ? <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
                            {cardDetails?.map((item) => (
                                <>
                                    {loader ?
                                        <ProjectShimmer></ProjectShimmer> :
                                        <div className="border rounded-2xl project-card bg-primary-content p-[18px] transform transition-transform duration-500 hover:scale-[1.03]	">
                                            <div className='flex justify-between'>
                                                <img
                                                    src={item.publisherLogo ? item.publisherLogo : DefaultImg}
                                                ...
                                                <div className='flex gap-2'>
                                                    ...
                                                    {item.telegram && (
                                                        <span
                                                            ...
                                                            onClick={() => handleWebSiteLinks(item, 'TELEGRAM')}
                                                        ></span>
                                                    )}
                                                    {item.discard && (
                                                        <span
                                                            className="icon discord-md cursor-pointer"
                                                            onClick={() => handleWebSiteLinks(item, 'DISCARD')}
                                                        ></span>
                                                    )}
                                                </div>
                                            </div>
                                            <div className="">
                                                ...
                                                ><span className="text-lg font-semibold text-secondary">{item?.projectName}</span></Link>
                                                <div className=" flex gap-6">
                                                    <p className="text-warning-content">
                                                        <span className={`inline-block w-2 h-2 rounded-full ${statusColourList[item?.publicOrPrivateStatus]}`}></span>{' '}
                                                        {item?.publicOrPrivateStatus}
                                                    </p>

                                                    <p className="text-warning-content">
                                                        <span className={`icon  ${item?.tokenNetworkLogo || 'usdt icon scale-[1.4]'} `}></span>{' '}
                                                        {item?.tokenNetwork ? item?.tokenNetwork : process.env.REACT_APP_CURRENCY}
                                                    </p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="">
                                                <div className="">
                                                    ...
                                                    <div className="pt-2">
                                                        <p className="text-secondary text-3lines min-h-[72px]">{item?.description}</p>
                                                        <div className={`bg-base-300 rounded-2xl p-2.5 my-2`}>
                                                            <label className={`text-base-200 text-base font-medium`}>Cast & Crew</label>
                                                            {item.cast_Crews?.length == 0 ? <p className='text-base font-normal text-secondary h-12 overflow-hidden'>-</p> : <p className='text-base font-normal text-secondary h-12 overflow-hidden'>{item.cast_Crews.map(obj => obj['name']).join(', ')}</p>}
                                                        </div>

                                                        <div className='flex justify-between'>
                                                            <label className={`text-base-200 text-base font-semibold`}>Total Raise</label>
                                                            <p className='text-base font-normal text-secondary'>
                                                                {item.totalRaised?.toLocaleString() ? '$' : ''}
                                                                {item?.totalRaised?.toLocaleString() || '-'}</p>
                                                        </div>
                                                        <div className='flex justify-between my-2'>
                                                            <label className={`text-base-200 text-base font-semibold `}>Total Supply</label>
                                                            <p className='text-base font-normal text-secondary'>
                                                        ...
                                                        <div className='flex justify-between'>
                                                            <label className={`text-base-200 text-base font-semibold `}>Price</label>
                                                            <p className='text-base font-normal text-secondary'>
                                                                {item?.tokenVolume} {item?.tokenSymbol} = {item?.paymentValue?.toLocaleString()}{' '}
                                                                {item?.paymentSymbol}
                                                            </p>
                                                        </div>
                                                        <div className={`bg-secondary-content rounded-2xl mt-3 flex`}>
                                                            <div className='w-full my-2.5 px-3'>
                                                                <label className={`text-base-100 opacity-60 text-base font-medium`}>
                                                                    {item?.accessType == 'Private' ? 'Private Opens' : 'Public Opens'}
                                                                </label>
                                                                <p className='text-base font-normal text-base-100'>
                                                                    {item.accessType == 'Private' ?
                                                                        item?.privateStartDate == ' ' ? 'TBA'
                                                                            : moment(item.privateStartDate).format("DD-MM-YYYY HH:mm")
                                                                        : moment(item.publicStartDate).format("DD-MM-YYYY HH:mm")
                                                                    }(UTC)
                                                                </p>
                                                            </div>
                                                            <div className={`border-l border-base-200 w-full my-2.5 pr-3 pl-4`}>
                                                                <label className={`text-base-100 opacity-60 text-base font-medium`}>
                                                                    {item?.accessType == 'Private' ? 'Private Closes' : 'Public Closes'}
                                                                </label>
                                                                <p className='text-base font-normal text-base-100'>
                                                                    {item.accessType == 'Private' ?
                                                                        item?.privateEndDate == ' ' ? 'TBA'
                                                                            : moment(item.privateEndDate).format("DD-MM-YYYY HH:mm")
                                                                        : moment(item.publicEndDate).format("DD-MM-YYYY HH:mm")
                                                                    }(UTC)
                                                                </p>
                                                            </div>
                                                        </div>
                     ...
                     {(cardDetails?.length != 0 && loadData) && loader&&  <div className="text-center mt-2">
                    <Spinner className="text-center"></Spinner>
                </div>}
                        {(cardDetails?.length != 0 && loadData)&&!loader && <div className='text-center mt-4' > <span onClick={fetchMoreData} className='cursor-pointer text-base text-primary font-semibold'>See More</span><span onClick={fetchMoreData} className='mx-auto block icon see-more cursor-pointer mt-[-4px]'></span> </div>}
