...
const FoundingMember = (props) => {
  ...
  return (
    <>
      {loader && <ProjecViewFoundingmembersShimmer />}
      {!loader && (!data?.stakersData || data?.stakersData?.length === 0) && (
        <></>
      )}
      {!loader && data?.stakersData && data?.stakersData?.length > 0 && <h4 className={`text-2xl font-semibold text-secondary mb-2 mt-8`}>
            Investors
          </h4>}
      {!loader && data?.stakersData && data?.stakersData?.length > 0 && (
            ...
            {data?.stakersData?.map((item, index) =>
              index < 4 ? (
                ...
                >
                  ...
                  <p
                    className={`text-base font-normal  text-secondary text-center text-ellipsis overflow-hidden truncate`}
                    title={item.userName}
                  >
                    {item.userName || "--"}
                  </p>
