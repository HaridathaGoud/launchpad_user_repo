...
const CastAndCrewMember = (props) => {
    return (
        <>
            <div className={`md:gap-4 flex gap-6 justify-between items-start overflow-x-auto`}>

                    {props.castCrewsData?.map((item,index)=>
                    (index<5 ? <div className='max-lg:min-w-[128px]'> <div className={`relative w-20 h-20 mx-auto overflow-hidden rounded-full`}>
                            <img src={item.image?item.image:member} className={`rounded-full h-full w-full object-cover`} alt='' />
                        </div>
                        <p className={`text-base font-normal  text-secondary text-center`}>{item.name}</p>
                        <p className={`text-info text-base font-normal text-center`}>as {item.role}</p>
                        </div> : "" )  )}
                    
              {props.castCrewsData?.length!=0&&props.castCrewsData?.length>= 5 && <Link to={`/castcrewsmembersview/${props.pjctId}`}>
               <div className={`bg-secondary w-20 h-20 rounded-full shrink-0 flex items-center justify-center cursor-pointer ml-0 md:ml-10`}>
                ...
                {props.castCrewsData?.length==0&&<div className="text-center flex-1">
