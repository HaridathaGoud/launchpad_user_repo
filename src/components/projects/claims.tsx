            ...
            {!claimsLoader && (
                <>
                    ...
                    {claimHide && (
                        <div className="">
                            <div>
                                <h2 className="text-2xl font-medium"><span className='text-secondary'>Cl</span><span className='text-primary'>aim</span></h2>
                            </div>
                            <div className="">
                                <p className="text-base text-neutral">
                                    1. You first need to purchase the tokens on this page, when the{' '}
                                    {process.env.REACT_APP_OFFERING_TITLE}’S is live on TBA
                                </p>
                                <p className="text-base text-neutral">
                                ...
                                <table className='refferal-table md:w-full border-spacing-y-2.5 border-separate max-sm:w-[800px] ' >
                                    <thead>
                                        <tr>
                                            ...
                                            <th className='text-left text-base text-secondary font-bold'>Allocation</th>
                                            <th className='text-left text-base text-secondary font-bold'>Date</th>
                                            <th className='text-left text-base text-secondary font-bold'>Claimed</th>
                                    ...
                                    <>
                                        {claimsData?.map((claims: any, index: any) => (
                                            <>
                                                <tbody key={index}>
                                                    <tr>
                                                        <td><p className='font-normal text-sm text-secondary'>{index + 1}</p> </td>
                                                        <td><p className='font-normal text-sm text-secondary'>{claims.allocation.toLocaleString()}</p> </td>
                                                        <td><p className='font-normal text-sm text-secondary'>{claims.date ? moment(claims.date).format("DD-MM-YYYY HH:mm") : "--"}{claims.date ? `(UTC)` : "--"}</p> </td>
                                                        <td><p className='font-normal text-sm text-secondary'>{claims?.claimedValue?.toLocaleString() || "0"}</p> </td>
                                                        <td className='!p-2 text-right'>
                                                            ...
                                                            {!claims?.isBuy && (
                                                                <Button
                                                                    ...
                                                                    disabled={claims.allocation === 0 || claims?.isBuy}
                                                            ...
                                                            {claims?.isBuy && (
                                                                <>
                                                                    {' '}
                                                                    <Button type='primary' btnClassName="!py-0 px-6" disabled={claims?.isBuy}>
                                                                        Claimed
                                                                    </Button>
                                                                </>
                                                            )}
