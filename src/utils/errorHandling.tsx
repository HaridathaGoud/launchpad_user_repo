const isErrorDispaly = (objValue: any) => {
  if (objValue?.data && typeof objValue?.data === "string") {
    ...
    typeof objValue?.response?.data === "object"
  ) {
    if (
      objValue?.response?.data?.error?.message
        .toLowerCase()
        .includes("modelstate")
    ) {
      ...
      objValue?.response?.data?.errors &&
      typeof objValue?.response?.data?.errors === "object"
    ) {
      const messages = Object.values(objValue?.response?.data?.errors) || [];
      let result = "";
      messages.forEach((message, indx) => {
        result += message + (indx !== messages.length - 1 ? ", " : "");
      });
      return result;
    ...
    if (
      objValue?.response?.data?.error?.message &&
      typeof objValue?.response?.data?.error?.message === "string"
    ) {
      return objValue?.response?.data?.error?.message;
    }
    ...
    // }
    if (
      objValue?.data?.error?.message &&
      typeof objValue?.data?.error?.message === "string"
    ) {
